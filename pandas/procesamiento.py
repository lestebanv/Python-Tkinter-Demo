...
def graficar(df_completo,var_x:str,var_y:str,titulo:str,x_label:str,y_label:str):
    fig=plt.figure()
    plano=fig.gca()
    plano.plot(df_completo[var_x],df_completo[var_y])
    plano.set_title(titulo, fontsize = 20)
    plano.set_xlabel(x_label, fontsize = 20)
    plano.set_ylabel(y_label, color = 'blue', fontsize = 24)
    plano.grid(True)
    return(fig)
